  ...
  try {
    for (const s of schemas()) {
      try {
        ...
        if (n > 0) { console.log(`[init] db ${s.name}: ${n} tables present — skip`); detail.push(`${s.name}:${n}(kept)`); continue; }
        const ddl = fs.readFileSync(path.join(SQL_DIR, s.file), 'utf8');
        await conn.query(`USE \`${s.name}\``);
        await conn.query(ddl);
        const loaded = await tableCount(conn, s.name);
        console.log(`[init] db ${s.name}: loaded ${s.file} -> ${loaded} tables`);
        detail.push(`${s.name}:${loaded}(loaded)`);
      } catch (e) {
