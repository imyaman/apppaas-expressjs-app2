...
// Last ~n bytes of seafile.log, for remote diagnosis via /status.
export function tailLog(n = 1500) {
  try {
    const f = path.join(config.logDir, 'seafile.log');
  ...
  child.on('exit', async (code, sig) => {
    ...
    for (let i = 0; i < 20; i++) {
      ...
      if (pid && isAlive(pid)) {
        ...
        restarts = 0;
        status.seaf = 'running'; status.seafPid = pid; status.seafError = null;
        monitorDaemon(pid);
        return;
      }
